...
## Input/output
The script can accept FASTA files with one or multiple records as an input. The output of the script is PDF file with the results of protein digestion. Each file is autonamed according to this pattern: "<Enzyme_chosen>_peptides_<Digestion_mode>_from_<Protein_description_from_FASTA_header>". That means that user can easily distinguish the different files just by reading the name of the file. It already contains the nme of the enzyme which digests the target protein, digestion mode, and protein description from FASTA header.  Each file is in separate folder: "<Enzyme_chosen>_peptides_<Digestion_mode>_from_<Protein_description_from_FASTA_header>datetime". All result folders are in folder "results", which creates during the first run of the program.
